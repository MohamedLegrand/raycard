...
func (r *WalletRepository) FindByUtilisateurID(ctx context.Context, utilisateurID string) (*domain.Wallet, error) {
	return r.findOneBy(ctx, "utilisateur_id", utilisateurID)
}

// findOneBy : colonne provient toujours d'un appel interne codé en dur.
func (r *WalletRepository) findOneBy(ctx context.Context, colonne string, valeur any) (*domain.Wallet, error) {
	query := fmt.Sprintf(`
...
func (r *WalletRepository) UpdateSolde(ctx context.Context, w *domain.Wallet) error {
	...
	if tag.RowsAffected() == 0 {
		return domain.ErrWalletIntrouvable
	}
	return nil
}
