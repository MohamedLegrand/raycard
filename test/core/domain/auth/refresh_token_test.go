...
func TestNouveauRefreshToken(t *testing.T) {
	rt, err := auth.NouveauRefreshToken("user-1", "hash-abc", time.Hour)
...
func TestNouveauRefreshToken_DonneesInvalides(t *testing.T) {
	_, err := auth.NouveauRefreshToken("", "hash-abc", time.Hour)
	assert.ErrorIs(t, err, commun.ErrDonneesInvalides)

	_, err = auth.NouveauRefreshToken("user-1", "hash-abc", 0)
	assert.ErrorIs(t, err, commun.ErrDonneesInvalides)
}
