...
func TestNouveauTicketConnexion(t *testing.T) {
	tk, err := auth.NouveauTicketConnexion("user-1", "hash-ticket", "hash-code", 5, time.Hour)
...
func TestNouveauTicketConnexion_DonneesInvalides(t *testing.T) {
	_, err := auth.NouveauTicketConnexion("", "hash-ticket", "hash-code", 5, time.Hour)
	assert.ErrorIs(t, err, commun.ErrDonneesInvalides)

	_, err = auth.NouveauTicketConnexion("user-1", "hash-ticket", "hash-code", 0, time.Hour)
	assert.ErrorIs(t, err, commun.ErrDonneesInvalides)

	_, err = auth.NouveauTicketConnexion("user-1", "hash-ticket", "hash-code", 5, 0)
	assert.ErrorIs(t, err, commun.ErrDonneesInvalides)
}
...
func TestTicketConnexion_EstValide(t *testing.T) {
	...
	t.Run("tentatives épuisées", func(t *testing.T) {
		tk, err := auth.NouveauTicketConnexion("user-1", "hash-ticket", "hash-code", 2, time.Hour)
		require.NoError(t, err)
